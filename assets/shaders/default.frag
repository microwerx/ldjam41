...
float sobelEdge(float spread) {
  ...
  for (int i=0; i<3; i++) {
    for (int j=0; j<3; j++) {
        vec3 sample  = texture2D(GBufferColor0, uv + scale * vec2(i-1, j-1)).rgb;
        // I[i][j] = length(sample); 
        float depth  = 1.0 - texture2D(GBufferDepth, uv + scale * vec2(i-1, j-1)).r;
        float magnitude = length(sample);
        I[i][j] = depth * magnitude;
    }
  }
...
void main() {
  ...
  if (VizShowGBuffer != 0) {
    gl_FragColor = vec4(Fragment.N, 1.0);
    return;
  }
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (i == 0 && sunDirTo.y > 0.0)
      shadowAmount = GetSunShadow();

    if (uPBKsm != 0.0 && shadowAmount > 0.0)
    {
      float D = PBKsm >= 0.0 ? D_GTR(Lights[i].NdotH)
                            : D_BlinnPhong(Material.specularExponent, Lights[i].NdotH);
      float F = F_Schlick(Material.F0, Lights[i].LdotD);
      float G = G2_MaskingShadowing(Lights[i].NdotL, Fragment.NdotV, Lights[i].NdotH, Lights[i].VdotH);
      ...
      finalColor += specularColor * shadowAmount;
    }
    ...
    finalColor += diffuseColor * shadowAmount;
  }
